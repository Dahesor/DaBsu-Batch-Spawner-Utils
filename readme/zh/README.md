...
其逻辑与`nbt=`选择器参数相同。系统会提示你输入一个复合标签，所有包含该NBT的实体的刷怪笼都会被选中。例如，输入`{id:"minecraft:zombie"}`将选中所有生成僵尸的刷怪笼。
...
注意：如果你运行的指令修改了刷怪笼的NBT，则必须在编辑后从大别墅主菜单中运行**自检与优化**功能，以确保你所做的更改被大别墅系统正确同步。
