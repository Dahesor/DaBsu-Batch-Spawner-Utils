data modify storage dabsu:run dialog.dialog set value {type:"minecraft:multi_action_input_form",\
    ...
    actions:[\
        ...
        {id:"nbt",label:{text:"Select by nbt= Check",color:"#aeffff"},tooltip:"Select all spawners that has at least one entity passing a §anbt=§r check (like the one in entity selector argument) using the compound tag you input \n§aUnloaded spawners included\n§aWorks Across Dimensions",on_submit:{type:"command_template",template:"trigger trigger.dabsu set 1145$(none)"}},\
        {id:"same_type",label:{text:"Select Same Potential",color:"#aeffff"},tooltip:"Select all registered spawners that have the same §7SpawnPotentials§r as the spawner §byou are standing on§r. \n§aUnloaded spawners included \n§aWorks Across Dimensions",on_submit:{type:"command_template",template:"trigger trigger.dabsu set 113$(add)"},width:170},\
        {id:"all",label:{text:"Select All",color:"#aeffff"},tooltip:"Select all registered spawners. \n§aUnloaded spawners included\n§aWorks Across Dimensions",on_submit:{type:"command_template",template:"trigger trigger.dabsu set 114$(add)"},width:170},\
    ]\
}
