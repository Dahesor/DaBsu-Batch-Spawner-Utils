data modify storage dabsu:run dialog.dialog set value {type:"minecraft:multi_action",\
    ...
    body:[{type:"plain_message",width:400,contents:"Run any command. Context: positioned at each spawner, as each spawner's marker entity"},{type:"plain_message",width:400,contents:{text:"...",color:"gold"}}],\
    inputs:[\
...
data modify storage dabsu:run dialog.dialog.body[0].contents set from storage dabsu:run lang.this.temp_edit_any_command_1
data modify storage dabsu:run dialog.dialog.body[1].contents.text set from storage dabsu:run lang.this.run_command_warn
