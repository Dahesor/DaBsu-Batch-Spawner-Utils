data modify storage dabsu:run dialog.dialog set value {type:"confirmation",\
    title:[{text:"[DaBsu] ",color:"gold"},{text:"Quick Edit",color:"light_purple"}," ",{text:"Sync Changes",color:"white"}],\
    can_close_with_escape:true,after_action:"close",pause:false,\
    yes:{label:{translate:"gui.yes",color:"green"},action:{type:"run_command",command:"trigger trigger.dabsu set 182"}},\
    no:{label:{translate:"gui.cancel"}},\
    body:[\
        {type:"plain_message",contents:"§l========§a§l Sync Changes §f§l========",width:400},\
...
data modify storage dabsu:run dialog.dialog.title[3].text set from storage dabsu:run lang.this.menu_templates_revert_confirm_1
data modify storage dabsu:run dialog.dialog.body[0].contents set from storage dabsu:run lang.this.menu_templates_sync_confirm_1
...
data modify storage dabsu:run dialog.dialog.body[4].contents set from storage dabsu:run lang.this.menu_templates_sync_confirm_2
